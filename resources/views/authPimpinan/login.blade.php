		...
		<div id="particles-js">
            <div class="loginBox">
                ...
                @if ($errors->any())
                    ...
                    @else
                    <h6>Form Login Pimpinan</h6>
                    <p style="text-align:center; margin-bottom:20px;">Sistem Pelaporan Kegiatan Harian<br> Tendik Oleh Pimpinan</p>
                @endif
                <form method="post" action="{{ route('pimpinan.login') }}">
                    @csrf
                    <p>Username</p>
                    <input type="text" value="{{ old('pasNama') }}" name="pasNama" placeholder="masukan username">
                    <p>Password</p>
                    <input type="password" name="password" placeholder="••••••">
                    <button type="submit" name="submit" style="margin-bottom:10px;r"><i class="fa fa-sign-in"></i>&nbsp; Login</button>
                    <a href="{{ route('login') }}" style="font-weight:100; font-size:14px" ><i class="fa fa-sign-in"></i>&nbsp;&nbsp;Login Tendik</a>
                </form>
            </div>
        </div>
    </body>
