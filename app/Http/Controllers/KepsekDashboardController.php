...
class KepsekDashboardController extends Controller
{
    ...
    public function dashboard(){
        ...
        return view('kepsek.dashboard',compact('karyawans'));
    }
    ...
    public function update(Request $request){
        ...
        $attributes = [
            'komenKepsek'   =>  'Komentar Kepala Sekolah',
        ];
        $this->validate($request, [
            'komenKepsek'    =>  'required',
        ],$messages,$attributes);
        // return $request->all();

        KegiatanTendik::where('kegId',$request->kegId)->update([
            'kegSaranKepsek'    =>  $request->komenKepsek,
        ]);
        $notification = array(
        ...
        $data['detail'] = DB::table('tbkegiatan')
                        ...
                        ->first();
        $karyawans = DB::table('tbpegawai')->select('pegNip','pegNama')->where('pegJenisKepeg','Karyawan')->get();
        return view('kepsek.dashboard')->with('success','Komentar berhasil ditambahkan')->with('data',$data)->with('today',$today)->with('karyawans',$karyawans);
    }
}
